...
@section('title', 'Ride Details #' . $ride->id . ' - Taxi-At-Foot')
...
<script>
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        const currentStatus = "{{ $ride->status->value }}";
        ...
        if (currentStatus === 'pending') {
            ...
            // If using Laravel Echo to listen for status updates, we could clear this interval.
            // But a simple page reload approach by the user is likely fine for now.
        }

        // Listen for live specific driver location if accepted
        @if(in_array($ride->status->value, ['accepted', 'driver_arriving', 'in_progress']) && $ride->driver_id)
            ...
            function fetchDriverLocation() {
                fetch('{{ route("customer.rides.driver-location", $ride) }}')
                    .then(res => res.json())
                    .then(data => {
            ...
            if (typeof window.Echo !== 'undefined') {
                window.Echo.channel(`ride.{{ $ride->id }}`)
