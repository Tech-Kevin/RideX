    ...
    document.addEventListener('DOMContentLoaded', function () {
        if (typeof window.Echo !== 'undefined') {
            window.Echo.channel('rides')
                .listen('.ride.requested', (e) => {
                    const noRidesMsg = document.getElementById('no-rides-message');
                    if (noRidesMsg) {
                        noRidesMsg.remove();
                    }

                    // For formatting money and id
                    const formatPrice = new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(e.ride.estimated_fare);
                    const formattedId = e.ride.id.toString().padStart(5, '0');

                    const container = document.getElementById('rides-container');
                    const newCard = document.createElement('div');
                    newCard.className = 'bg-white border border-emerald-200 rounded-3xl p-6 shadow-lg relative overflow-hidden flex flex-col group ring-2 ring-emerald-500/20 transform transition-all animate-[fade-in-down_0.5s_ease-out]';
                    newCard.id = `ride-card-${e.ride.id}`;
                    
                    newCard.innerHTML = `
                        <div class="absolute top-0 right-0 w-32 h-32 bg-emerald-400/20 rounded-full blur-[30px] -mt-10 -mr-10 pointer-events-none"></div>
                        <div class="absolute top-0 left-0 w-full h-1 bg-emerald-400"></div>

                        <div class="flex justify-between items-start mb-6 border-b border-neutral-100 pb-4 mt-2">
                            <div>
                                <span class="inline-block px-2 py-0.5 bg-emerald-100 text-emerald-700 font-bold font-mono text-[10px] uppercase tracking-widest rounded mb-1 border border-emerald-200">
                                    #${formattedId}
                                </span>
                                <p class="text-xs text-emerald-500 font-bold animate-pulse">Just Now!</p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] font-black tracking-widest uppercase text-emerald-600 mb-0.5">Estimated Payout</p>
                                <p class="text-2xl font-black font-heading text-neutral-900 tracking-tighter">${formatPrice}</p>
                            </div>
                        </div>
                        ...
                        <div class="relative pl-7 space-y-5 mb-auto">
                            ...
                            <div class="relative">
                                <div class="absolute -left-[2.1rem] w-4 h-4 rounded-full bg-emerald-400 border-[3px] border-white shadow-sm ring-1 ring-emerald-400/20"></div>
                                <p class="text-[10px] font-black text-emerald-600 tracking-widest uppercase mb-1">Pickup</p>
                                <p class="text-sm font-bold text-neutral-800 leading-snug line-clamp-2" title="${e.ride.pickup_address}">${e.ride.pickup_address}</p>
                            </div>
                            <div class="relative">
                                <div class="absolute -left-[2.1rem] w-4 h-4 bg-amber-400 border-[3px] border-white shadow-sm ring-1 ring-amber-400/20 transform rotate-45"></div>
                                <p class="text-[10px] font-black text-amber-500 tracking-widest uppercase mb-1">Drop-off</p>
                                <p class="text-sm font-bold text-neutral-800 leading-snug line-clamp-2" title="${e.ride.drop_address}">${e.ride.drop_address}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between items-center mt-6 py-4 border-t border-neutral-100">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="text-[10px] font-black tracking-widest uppercase text-neutral-400 leading-none mb-0.5">Rider</p>
                                    <p class="text-xs font-bold text-neutral-900">${e.customer_name || 'Guest'}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] font-black tracking-widest uppercase text-neutral-400 leading-none mb-0.5">Distance</p>
                                <p class="text-sm font-bold font-mono text-neutral-900">${parseFloat(e.ride.distance_km).toFixed(2)} km</p>
                            </div>
                        </div>

                        <form method="POST" action="/driver/rides/${e.ride.id}/accept" class="mt-4">
                            @csrf
                            <button type="submit" class="w-full py-3.5 bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-800 font-bold rounded-xl shadow-lg shadow-neutral-900/20 transition-all hover:-translate-y-0.5 active:scale-[0.98] flex items-center justify-center gap-2">
                                Accept Ride
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
